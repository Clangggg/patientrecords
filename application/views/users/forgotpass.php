...
<?php if($this->session->flashdata('forgotpass_failed')): ?>
  ...
  <h1 class="h4 text-gray-900 mb-4 text-center">Forgot Your Password?</h1> 
       ...
       <?php echo form_open('indexcontrol/forgotpassuser', $attributes); ?>
        ...
        <div style="margin-left: 45px;" class="form-group row">
          <?php echo form_label('Username') ?>
            <input type="text" class="form-control <?= (form_error('user') == "" ? '':'is-invalid') ?>"  name="user" placeholder="Enter your username" >
          <div class="text-danger text-center"><?php echo form_error('user'); ?></div>
        </div>
        <button style="margin-left: 149px;" type="submit" class="btn btn-primary mb1 bg-blue mt-4" name="submit">
